...
def main():
    ...
    plt.savefig("./plots/" + str(dims) + "_normal_sample")
    plt.clf()

    plt.imshow(x_train[:,5253].reshape((dims,dims)))
    plt.title(classes[int(y_train[0][5253])])
    plt.savefig("./plots/" + str(dims) + "_pneumonia_sample")
    plt.clf()
    ...
    plt.savefig("./plots/" + str(dims) + "_learn")
...
def load_dataset(dims):
    ...
    x_test = pickle.load(open("./coronahack/" + str(dims) + "/x_test.p","rb"))
    y_test = pickle.load(open("./coronahack/" + str(dims) + "/y_test.p","rb"))
